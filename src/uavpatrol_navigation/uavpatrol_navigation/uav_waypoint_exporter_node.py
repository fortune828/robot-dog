...
class UavWaypointExporterNode(Node):
    ...
    def __init__(self):
        ...
        default_output = str(project_root / DEFAULT_PLANNING.route_output_dir)
        ...
        self.declare_parameter("altitude_m", DEFAULT_PLANNING.altitude_m)
        self.declare_parameter("altitude_mode", DEFAULT_PLANNING.altitude_mode)
        self.declare_parameter("speed_mps", DEFAULT_PLANNING.speed_mps)
        ...
        self.declare_parameter("auto_convert_to_dji", True)
        self.declare_parameter("auto_visualize", True)
        self.declare_parameter("semantic_copy_enabled", False)
        self.declare_parameter("map_file", str(project_root / DEFAULT_PLANNING.map_file))
        self.declare_parameter("patrol_area_file", str(project_root / DEFAULT_PLANNING.patrol_area_file))
        self.declare_parameter(
            "visualization_file",
            str(project_root / DEFAULT_PLANNING.visualization_file),
        )
        self.declare_parameter("dji_mission_name", "uav_area_patrol")
        self.declare_parameter("dji_finish_action", "go_home")
        self.declare_parameter("dji_wayline_id", 0)
        ...
        self._auto_convert_to_dji = bool(self.get_parameter("auto_convert_to_dji").value)
        self._auto_visualize = bool(self.get_parameter("auto_visualize").value)
        self._semantic_copy_enabled = bool(
            self.get_parameter("semantic_copy_enabled").value
        )
        self._map_file = self.get_parameter("map_file").value
        self._patrol_area_file = self.get_parameter("patrol_area_file").value
        self._visualization_file = self.get_parameter("visualization_file").value
        self._dji_mission_name = self.get_parameter("dji_mission_name").value
        self._dji_finish_action = self.get_parameter("dji_finish_action").value
        self._dji_wayline_id = int(self.get_parameter("dji_wayline_id").value)
    ...
    def _export_path(self, msg):
        ...
        if self._semantic_copy_enabled:
            semantic_json_path = self._output_dir / "semantic_uav_waypoints.json"
            semantic_csv_path = self._output_dir / "semantic_uav_waypoints.csv"
            semantic_json_path.write_text(
                json.dumps(payload, ensure_ascii=False, indent=2) + "\n",
                encoding="utf-8",
            )
            self._write_csv(semantic_csv_path, waypoints)
        ...
        if self._auto_convert_to_dji:
            draft_path, kmz_path, log_path = convert(
                json_path,
                self._output_dir,
                mission_name=self._dji_mission_name,
                finish_action=self._dji_finish_action,
                wayline_id=self._dji_wayline_id,
            )
            self.get_logger().info(
                f"Converted DJI mission files: {draft_path}, {kmz_path}, {log_path}"
            )
        if self._auto_visualize:
            try:
                html_path, _, _, _ = create_visualization(
                    ...
                    output_file=self._visualization_file,
                )
                self.get_logger().info(f"Generated patrol visualization: {html_path}")
            except Exception as exc:
                self.get_logger().warn(f"Patrol visualization skipped: {exc}")
