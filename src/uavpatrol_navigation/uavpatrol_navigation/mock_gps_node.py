...
class MockGpsNode(Node):
    ...
    def __init__(self):
        ...
        self._current_yaw = 0.0        # 当前航向角（弧度）
        
        # ========== 点到点直线追踪状态变量 ==========
        self._target_idx = 0           # 当前目标路径点索引
        self._current_x = 0.0          # 当前 X 坐标（局部平面）
        self._current_y = 0.0          # 当前 Y 坐标（局部平面）
        self._odom_x = 0.0
        self._odom_y = 0.0
        self._odom_yaw = 0.0
    ...
    def _path_callback(self, msg: Path):
        ...
        self.get_logger().info(
            ...
            f"Mock UAV will move from current position...\033[0m"
        )

        self._path_poses = list(msg.poses)
        self._path_done = False
        self._goal_reported = False  # 新路径到达，重置上报标记
        self._target_idx = 0  # 路径第 0 个点是第一个目标
        self._log_counter = 0  # 重置限频日志计数器

        if not self._has_path:
            self._has_path = True
            self.get_logger().info(
                f"\033[32m[SUCCESS] Global path received! "
                f"{len(self._path_poses)} waypoints, "
                f"moving from current position...\033[0m"
            )
        else:
            self.get_logger().info(
