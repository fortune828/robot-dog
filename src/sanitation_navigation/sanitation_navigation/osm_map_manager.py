"""osm_map_manager.py — OSM 地图引擎节点

职责:
  1. 加载 OSM 地图，构建路网有向图
  2. RViz2 矢量可视化（路网 + 建筑）
  3. 离线路由服务 /plan_osm_path
  4. 独立 P2P 导航（订阅 /goal_pose → /global_plan）
...
# ---------------------------------------------------------------------------
#  Coordinate conversion (Equirectangular projection)
#  公式与 mock_gps_node / gaode_path_proxy 完全一致
# ---------------------------------------------------------------------------

def _latlon_to_local(lat, lon, origin_lat, origin_lon):
    """WGS-84 (lat, lon) → local ENU (x, y) in meters."""
...
def _local_to_latlon(x, y, origin_lat, origin_lon):
    """Local ENU (x, y) → WGS-84 (lat, lon)."""
    cos_phi = math.cos(math.radians(origin_lat))
    lon = origin_lon + x / (111194.9 * cos_phi)
    lat = origin_lat + y / 111194.9
    return lat, lon
...
class OsmMapManager(Node):
    ...
    def __init__(self):
        ...
        if not HAS_PLANPATH_SRV:
            self.get_logger().fatal("PlanPath srv 未生成，请先 colcon build sanitation_interfaces")
        ...
        if not map_file:
            ...
            for parent in [p] + list(p.parents):
                candidate = parent / "data" / "map.osm"
                if candidate.is_file():
                    map_file = str(candidate)
                    break
            if not map_file:
    ...
    def _plan_route(self, start_x, start_y, goal_x, goal_y):
        ...
        """
        # ENU → WGS-84
        start_lat, start_lon = _local_to_latlon(
